...
import { updateComponent, selectComponent, removeComponent } from "../store/builderSlice";
import { 
  TrashIcon, ChevronDownIcon, ChevronUpIcon, SwatchIcon, 
  AdjustmentsHorizontalIcon, QueueListIcon, DevicePhoneMobileIcon, 
  DeviceTabletIcon, ComputerDesktopIcon, PhotoIcon, CubeIcon, 
  ArrowsPointingOutIcon, ArrowsPointingInIcon, PaintBrushIcon, 
  CodeBracketIcon, SparklesIcon, Bars3CenterLeftIcon, 
  Squares2X2Icon, ViewfinderCircleIcon, PlayIcon, 
  DocumentTextIcon
...
const SpacingVisualizer = ({ component, onChange }) => {
  ...
  return (
    <div className="w-full bg-gray-50/80 p-1.5 rounded-2xl relative border border-gray-200 flex justify-center items-center py-5">
      <Typography className="absolute top-1.5 left-3 text-[8px] font-black text-gray-400 uppercase tracking-[0.2em]">Margin</Typography>
      ...
      <div className="flex flex-col items-center gap-1">
        ...
        <div className="flex items-center gap-1 w-full justify-center">
          ...
          <div className="bg-white p-2 rounded-xl border border-gray-200/60 shadow-sm flex flex-col items-center gap-1 w-40 relative">
            <Typography className="absolute top-1.5 left-2 text-[8px] font-black text-gray-400 uppercase tracking-[0.2em]">Padding</Typography>
            <div className="mt-3"><InputNode prop="paddingTop" /></div>
...
const SECTIONS = [
  {
    id: "layout", title: "Layout", icon: ViewfinderCircleIcon,
    controls: [
      { label: "Display", name: "display", type: "select", options: ["block", "flex", "grid", "inline-block", "none"] },
      { label: "Position", name: "position", type: "select", options: ["static", "relative", "absolute", "fixed"] },
      { label: "Z-Index", name: "zIndex" },
      { label: "Overflow", name: "overflow", type: "select", options: ["visible", "hidden", "scroll", "auto"] },
      { label: "Top", name: "top" },
  ...
  {
    id: "spacing", title: "Spacing", icon: ArrowsPointingInIcon,
    custom: true
  },
  {
    id: "size", title: "Size", icon: ArrowsPointingOutIcon,
    controls: [
  ...
  {
    id: "typography", title: "Typography", icon: Bars3CenterLeftIcon,
    controls: [
      { label: "Font", name: "fontFamily", placeholder: "Poppins, sans-serif" },
      { label: "Weight", name: "fontWeight", type: "select", options: ["100","200","300","400","500","600","700","800","900","normal","bold"] },
      { label: "Size", name: "fontSize" },
      { label: "Height", name: "lineHeight" },
      { label: "Space", name: "letterSpacing" },
      { label: "Color", name: "color", type: "color" },
      { label: "Align", name: "textAlign", type: "select", options: ["left", "center", "right", "justify"] },
      { label: "Transform", name: "textTransform", type: "select", options: ["none", "capitalize", "uppercase", "lowercase"] },
      { label: "Decor", name: "textDecoration", type: "select", options: ["none", "underline", "line-through"] }
    ]
  },
  {
    id: "background", title: "Background", icon: PaintBrushIcon,
    controls: [
      { label: "Color", name: "backgroundColor", type: "color" },
      { label: "Image", name: "backgroundImage", placeholder: "url(...)" },
      { label: "Gradient", name: "background", placeholder: "linear-gradient(...)" },
      { label: "Size", name: "backgroundSize", type: "select", options: ["auto", "cover", "contain"] },
      { label: "Position", name: "backgroundPosition" },
      { label: "Repeat", name: "backgroundRepeat", type: "select", options: ["repeat", "no-repeat", "repeat-x", "repeat-y"] },
    ]
  },
  {
    id: "border", title: "Border", icon: SwatchIcon,
    controls: [
      { label: "Radius", name: "borderRadius" },
      { label: "Width", name: "borderWidth" },
      { label: "Style", name: "borderStyle", type: "select", options: ["none", "solid", "dashed", "dotted"] },
      { label: "Color", name: "borderColor", type: "color" },
      { label: "Outline", name: "outline" },
    ]
  },
  {
    id: "effects", title: "Effects", icon: SparklesIcon,
    controls: [
      { label: "Box Shadow", name: "boxShadow", placeholder: "0px 4px 10px rgba(0,0,0,0.1)" },
  ...
  {
    id: "flexbox", title: "Flexbox", icon: QueueListIcon,
    controls: [
      { label: "Direction", name: "flexDirection", type: "select", options: ["row", "row-reverse", "column", "column-reverse"] },
      { label: "Justify", name: "justifyContent", type: "select", options: ["flex-start", "center", "flex-end", "space-between", "space-around"] },
      { label: "Align Items", name: "alignItems", type: "select", options: ["flex-start", "center", "flex-end", "stretch", "baseline"] },
      { label: "Wrap", name: "flexWrap", type: "select", options: ["nowrap", "wrap", "wrap-reverse"] },
  ...
  {
    id: "grid", title: "Grid", icon: Squares2X2Icon,
    controls: [
  ...
  {
    id: "transform", title: "Transform", icon: CubeIcon,
    controls: [
      { label: "Transform", name: "transform", placeholder: "scale(1.1) rotate(5deg)" },
      { label: "Origin", name: "transformOrigin" },
    ]
  },
  {
    id: "animation", title: "Animation", icon: PlayIcon,
    controls: [
...
const StyleControl = ({ control, component, onChange }) => {
  const { label, name, type = "text", options = [], placeholder = "auto" } = control;
  const val = component.styles?.[name] || "";

  if (type === "color") {
    return (
      <div className="flex items-center justify-between gap-3 mb-2">
          <Typography className="text-[9.5px] font-bold text-gray-400 uppercase tracking-widest w-1/3 truncate">{label}</Typography>
          <div className="flex-1 flex bg-white border border-gray-100 rounded-md overflow-hidden relative group hover:border-indigo-400 focus-within:border-indigo-500 transition-all shadow-sm min-h-[28px]">
              ...
              />
              <div className="w-6 h-full border-r border-gray-100 shrink-0" style={{ backgroundColor: val || "transparent" }}></div>
              <input 
                type="text" 
                value={val}
                onChange={(e) => onChange(name, e.target.value)}
  ...
  return (
    <div className="flex items-center justify-between gap-3 mb-2">
       <Typography className="text-[9.5px] font-bold text-gray-400 uppercase tracking-widest w-1/3 truncate" title={label}>{label}</Typography>
       <input 
         type="text" 
         value={val}
         onChange={(e) => onChange(name, e.target.value)}
...
const RightPanel = () => {
  const { currentPage, selectedComponentId, deviceView } = useSelector((state) => state.builder);
  const dispatch = useDispatch();
  const [activeTab, setActiveTab] = useState("Style");
  
  // Default open state for first few sections
  const [openSections, setOpenSections] = useState({
    layout: true, spacing: true, size: true, typography: false, background: false, border: false
  });
  ...
  const selectedComponent = currentPage ? findComponent(currentPage.layout, selectedComponentId) : null;
  ...
  if (!selectedComponent) {
    return (
      <div className="w-[340px] border-l border-gray-100 bg-white h-[calc(100vh-80px)] flex flex-col items-center justify-center p-8 text-center bg-gray-50/20 shrink-0 z-10 shadow-[-20px_0_40px_-20px_rgba(0,0,0,0.05)]">
        ...
        <Typography variant="h5" className="text-gray-900 font-extrabold tracking-tight mb-2">Style Editor</Typography>
  ...
  const SectionHeader = ({ section, isOpen, onToggle }) => (
    ...
    >
      <div className="flex items-center gap-3">
        {section.icon && <section.icon className={`h-4 w-4 ${isOpen ? 'text-indigo-600' : 'text-gray-400'}`} />}
        <Typography className={`text-[10px] font-black uppercase tracking-[0.2em] transition-colors ${isOpen ? 'text-indigo-900' : 'text-gray-500'}`}>
          {section.title}
        </Typography>
      </div>
      {isOpen ? <ChevronUpIcon className="h-3 w-3 text-indigo-400" strokeWidth={3} /> : <ChevronDownIcon className="h-3 w-3 text-gray-300" strokeWidth={3} />}
    </button>
  );
    ...
    <div className="w-[340px] border-l border-gray-100 bg-white h-[calc(100vh-80px)] flex flex-col overflow-hidden font-sans shadow-[-20px_0_40px_-20px_rgba(0,0,0,0.08)] shrink-0 z-10">
      ...
      <div className="px-5 py-3 border-b border-gray-100 bg-gray-50/30 flex items-center justify-between shrink-0">
         ...
         <div className="flex bg-white border border-gray-100 rounded-lg p-0.5 shadow-sm">
            <button className={`p-1.5 rounded-md transition-all ${deviceView === 'desktop' ? 'bg-indigo-50 text-indigo-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}>
               <ComputerDesktopIcon className="h-3.5 w-3.5" />
            </button>
            <button className={`p-1.5 rounded-md transition-all ${deviceView === 'tablet' ? 'bg-indigo-50 text-indigo-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}>
               <DeviceTabletIcon className="h-3.5 w-3.5" />
            </button>
            <button className={`p-1.5 rounded-md transition-all ${deviceView === 'mobile' ? 'bg-indigo-50 text-indigo-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}>
      ...
      <div className="flex border-b border-gray-100 shrink-0 bg-white shadow-sm z-10 w-full relative">
        {[
          ...
          >
            ...
            {activeTab === tab.id && <div className="absolute bottom-0 inset-x-0 h-0.5 bg-indigo-600" />}
          </button>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide bg-gray-50/10">
        ...
        {activeTab === "Style" && (
          <div className="pb-32">
             <div className="p-4 bg-indigo-600 text-white flex items-center justify-between shadow-inner">
                 <Typography className="text-[10px] font-black uppercase tracking-widest">State Editor</Typography>
                   ...
                   {openSections[section.id] && (
                     <div className="p-5 animate-in fade-in duration-300 bg-white">
                        ...
                        {section.custom && section.id === "spacing" && (
                           <SpacingVisualizer component={selectedComponent} onChange={handleStyleChange} />
                        )}
                     </div>
                   )}
                </div>
        ...
        {activeTab === "Settings" && (
          <div className="p-6 space-y-6 pb-32">
             <Typography className="text-[11px] font-black text-gray-900 uppercase tracking-[0.2em] border-b border-gray-100 pb-3">Component Settings</Typography>
             ...
             {selectedComponent.type === "image" && (
               <div className="space-y-3 p-5 bg-indigo-50/50 rounded-2xl border border-indigo-100/50">
                  <Typography className="text-[10px] font-black text-indigo-600 uppercase tracking-widest flex items-center gap-2"><PhotoIcon className="h-4 w-4" /> Asset URL</Typography>
                  <input 
          ...
          <div className="p-6 space-y-6 pb-32">
             <div className="p-6 bg-gradient-to-br from-gray-900 to-indigo-900 rounded-[2rem] shadow-xl text-white space-y-4">
                 <div className="flex items-center gap-3 border-b border-white/10 pb-4">
                     ...
                     <Typography className="text-sm font-black tracking-tight">Tailwind Classes</Typography>
                 </div>
                 <Typography className="text-[11px] font-medium text-indigo-200 leading-relaxed">
                     Override generated styles by applying Tailwind utilities directly to this component.
                 </Typography>
                 <textarea
             ...
             <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 text-center space-y-2">
                <Typography className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Internal Component ID</Typography>
                <Typography className="text-xs font-mono font-bold text-gray-900">{selectedComponent.id.slice(0,18)}...</Typography>
             </div>
          </div>
        )}
      </div>
      ...
      <div className="p-5 border-t border-gray-200 bg-white sticky bottom-0 shrink-0 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)] z-20">
        <button
          onClick={() => {
            if (window.confirm(`Permanently remove this ${selectedComponent.type}?`)) {
